...
cpm = cpm(dge_norm_fltd) #dropped to ~8.7k for cpms10
cpm_all = cpm(dge_norm) #~32k reads total

#save cpm heatmaps for plotting in morpheus
#write.csv(as.matrix(cpm), fs::path(output_dir, "cpm_tables/Filtered/20241231_Nfekoseq_cpmTable_filt_cpms10_2samples.csv"))
#write.csv(as.matrix(cpm_all), fs::path(output_dir, "cpm_tables/Filtered/20241231_Nfekoseq_cpmTable_all.csv"))
...
p4<-ggplot(pca_res, aes(PC1, PC2, fill=Sample_Name)) + 
    geom_point(size=3, shape =21, color = "black") +
    theme(text = element_text(size=6),
          legend.position = "top")
p4

grid <-ggarrange(p1,p2,p3, nrow=1)

grid
# save plots
save_plot(fs::path(output_dir, "QC/PCA/Filt/23samples_cpms10_PCA.pdf"),
          grid,dpi=90, base_aspect_ratio =3)
